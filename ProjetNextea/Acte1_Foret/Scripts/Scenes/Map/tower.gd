...
func _ready() -> void:
	...
	if Global.playerDeath:
		$Player.global_position = $SpawnIfPlayerDeath.global_position
...
func _on_area_2d_teleport_player_body_entered(body: Node2D) -> void:
	if (body.is_in_group("player")):
		$Player.global_position = $SpawnGouluForBoss/Area2DTeleportPlayer/EmplacementToRespawn.global_position
		for groupGoulu in $SpawnGouluForBoss.get_children():
			if groupGoulu is Area2D:
				for collision_shape in groupGoulu.get_children():
					if collision_shape is CollisionShape2D or collision_shape is CollisionPolygon2D:
